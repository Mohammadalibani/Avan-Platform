...
import { Plus, Edit, Trash2, RefreshCw, Download, Upload, Eye } from 'lucide-react';
...
const fieldsData = await fieldsRes.json();
        ...
        console.log('ℹ️ خطا در دریافت فیلدها، از آرایه خالی استفاده می‌شود');
        setFields([]);
      }

      // دریافت ادارات - ✅ مسیر اصلاح شد
      try {
        const deptsRes = await apiClient.get('/api/departments');
        setDepartments(deptsRes.data || []);
      } catch (deptsError) {
        console.log('ℹ️ خطا در دریافت ادارات');
        setDepartments([]);
      }

      // دریافت واحدها - ✅ مسیر اصلاح شد
      try {
        const unitsRes = await apiClient.get('/api/units-all');
        setUnits(unitsRes.data || []);
      } catch (unitsError) {
        console.log('ℹ️ خطا در دریافت واحدها');
        setUnits([]);
      }

      // دریافت دوره‌ها - ✅ مسیر اصلاح شد
      try {
        const periodsRes = await apiClient.get('/api/periods');
        setPeriods(periodsRes.data || []);
      } catch (periodsError) {
        console.log('ℹ️ خطا در دریافت دوره‌ها');
        setPeriods([]);
      }
    } catch (error) {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      await apiClient.put(`/personnel/${editingPersonnel.id}`, payload);
    ...
    <div>
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm mb-4 flex flex-wrap gap-3 items-center">
        ...
        <select
          ...
          onChange={(e) => {
            setFilters({ ...filters, department_id: e.target.value, unit_id: '' });
          }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  setFormData({ ...formData, department_id: e.target.value, unit_id: '' });
                }}
