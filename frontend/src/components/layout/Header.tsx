...
const Header: React.FC = () => {
  ...
  // ====== بارگذاری اعلان‌ها (با مدیریت خطا) ======
  useEffect(() => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          try {
            ...
            // اگر پاسخ JSON نبود، خطا نده
            console.log('ℹ️ API اعلان‌ها در دسترس نیست');
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    const loadMessages = async () => {
      try {
        const res = await fetch('/api/work/messages');
        if (res.ok) {
          const text = await res.text();
          try {
            const data = JSON.parse(text);
            ...
            console.log('ℹ️ API پیام‌ها در دسترس نیست');
          }
        }
      } catch (error) {
