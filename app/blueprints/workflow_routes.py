...
def update_workflow(workflow_id):
    ...
    updated_workflow = workflow_service.update(workflow_id, **update_data)
    
    return jsonify({
        'success': True,
        'message': 'Workflow updated successfully',
        'data': updated_workflow.to_dict()
    }), 200
...
@workflow_bp.route('/<int:workflow_id>/steps/<int:step_id>', methods=['PUT'])
@admin_required
def update_step(workflow_id, step_id):
    ...
    allowed_fields = ['name', 'description', 'sequence', 'action', 'role_name']
    update_data = {k: v for k, v in data.items() if k in allowed_fields}
    
    updated_step = workflow_service.update_step(step_id, **update_data)
    
    return jsonify({
        'success': True,
        'message': 'Step updated successfully',
        'data': updated_step.to_dict()
    }), 200


@workflow_bp.route('/<int:workflow_id>/steps/<int:step_id>', methods=['DELETE'])
@admin_required
