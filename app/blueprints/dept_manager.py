...
dept_manager_bp = Blueprint('dept_manager', __name__, url_prefix='/dept-manager')


# ==================== پنل مدیر اداره - صفحات ====================

@dept_manager_bp.route('/dashboard')
@login_required
def dashboard():
    ...
    # اگر ادمین است، یک اداره نمونه نشان بده
    if current_user.role == 'admin':
        dept_manager = DepartmentManager.query.first()
        if not dept_manager:
            flash('هیچ مدیر اداره‌ای در سیستم ثبت نشده است', 'warning')
            return render_template('dept_manager/dashboard.html', 
                                  department_name='اداره نمونه (نمایش ادمین)',
                                  period_title='دوره نمونه',
                                  today_date=jdatetime.datetime.now().strftime('%Y/%m/%d'),
                                  department_color='#667eea')
        dept_manager_id = dept_manager.id
    else:
        dept_manager = DepartmentManager.query.filter_by(user_id=current_user.id).first()
        if not dept_manager:
            flash('شما به هیچ اداره‌ای متصل نیستید. لطفاً با ادمین تماس بگیرید.', 'error')
            # ✅ تغییر: به جای ریدایرکت به dashboard، یک صفحه خطا نشان بده
            return render_template('dept_manager/dashboard.html', 
                                  department_name='شما به هیچ اداره‌ای متصل نیستید',
                                  period_title='تعریف نشده',
                                  today_date=jdatetime.datetime.now().strftime('%Y/%m/%d'),
                                  department_color='#667eea',
                                  error_message='شما به عنوان مدیر اداره ثبت شده‌اید، اما به هیچ اداره‌ای متصل نیستید. لطفاً با ادمین تماس بگیرید.')
        dept_manager_id = dept_manager.id
...
@dept_manager_bp.route('/test')
@login_required
def test():
    return "✅ مدیر اداره کار می‌کند!"
    
# ==================== APIهای مدیر اداره ====================

@dept_manager_bp.route('/api/all-data')
@login_required
def api_all_data():
...
@dept_manager_bp.route('/api/pending-requests')
@login_required
...
@dept_manager_bp.route('/api/delete-requests')
@login_required
...
@dept_manager_bp.route('/api/cancel-request', methods=['POST'])
@login_required
...
@dept_manager_bp.route('/api/request-add', methods=['POST'])
@login_required
def api_request_add():
    if current_user.role != 'dept_manager':
        return jsonify({'error': 'دسترسی غیرمجاز'}), 403
...
@dept_manager_bp.route('/api/request-delete', methods=['POST'])
@login_required
...
@dept_manager_bp.route('/api/personnel/batch-update', methods=['POST'])
@login_required
...
@dept_manager_bp.route('/export-excel')
@login_required
def export_excel():
    if current_user.role != 'dept_manager':
        flash('دسترسی غیرمجاز', 'error')
...
@dept_manager_bp.route('/api/personnel/<int:pid>')
@login_required
...
@dept_manager_bp.route('/api/personnel-history/<int:pid>')
@login_required
...
@dept_manager_bp.route('/api/personnel/<int:pid>/edit', methods=['PUT'])
@login_required
...
@dept_manager_bp.route('/api/department-color')
@login_required
def api_department_color():
    if current_user.role != 'dept_manager':
        return jsonify({'error': 'دسترسی غیرمجاز'}), 403
    dept_manager = DepartmentManager.query.filter_by(user_id=current_user.id).first()
    if not dept_manager:
...
@dept_manager_bp.route('/api/periods')
@login_required
def api_periods():
    if current_user.role != 'dept_manager':
        return jsonify({'error': 'دسترسی غیرمجاز'}), 403
...
@dept_manager_bp.route('/api/units/<int:unit_id>/personnel')
@login_required
def api_unit_personnel(unit_id):
    if current_user.role != 'dept_manager':
        return jsonify({'error': 'دسترسی غیرمجاز'}), 403
