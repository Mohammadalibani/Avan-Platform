...
def get_users():
    ...
    try:
        ...
        for u in pagination.items:
            try:
                # ✅ دریافت اطلاعات پرسنل با مدیریت خطا
                personnel = Personnel.query.filter_by(
                    national_code=u.national_code, 
                    is_deleted=False
                ).first()
                
                # ✅ متدهای با مدیریت خطا
                full_name = u.get_full_name() if hasattr(u, 'get_full_name') else f"{u.first_name} {u.last_name}"
                role_persian = u.get_role_persian() if hasattr(u, 'get_role_persian') else u.role
                created_date = u.get_jalali_created_date() if hasattr(u, 'get_jalali_created_date') else str(u.created_at)
                ...
                # ✅ مدیریت department_name و unit_name
                department_name = '-'
                unit_name = '-'
                if personnel:
                ...
                # ✅ اگر یک کاربر مشکل داشت، آن را نادیده بگیر
                print(f"⚠️ Error processing user {u.id}: {e}")
                continue
